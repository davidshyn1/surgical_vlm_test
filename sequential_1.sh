...
BACKEND=qwen2.5 DEVICE_VISIBLE=0 bash grounding_task.sh triplet_recognition_cholect50 --eval-protocol sequential_gt --prompt-mode mcq --eval-all --max-new-tokens 32
BACKEND=qwen2.5 DEVICE_VISIBLE=0 bash grounding_task.sh instrument_localization_endovis17 --viz
BACKEND=qwen2.5 DEVICE_VISIBLE=0 bash grounding_task.sh phase_recognition_cholec80
